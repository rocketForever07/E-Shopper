...
public interface CategoryRepository extends JpaRepository<Category,Integer> {
    ...
    @Query("SELECT new application.model.viewmodel.common.ChartDataVM(c.name,SUM(op.amount)) "+
            "FROM dbo_category c "+
            "JOIN c.listProducts p "+
            "JOIN p.orderProductList op "+
            "GROUP BY c.id")
    List<ChartDataVM> getListProductOrderedByCategory();

    @Query("select new application.model.viewmodel.common.CategoryVM(c.id,c.name,c.shortDesc,c.createdDate) "+
            "from dbo_category c")
    List<CategoryVM> getListCategories();
}
