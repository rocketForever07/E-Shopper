...
public class  ChartApiController {
    ...
    public DataApiResult getListFiveHotProduct(){
        ...
        Date date = new Date();
        List<ChartDataVM> chartDataVMS = productRepository.getListFiveHotProducts(pageable, date.getMonth() +1, date.getYear() + 1900);
    ...
    @GetMapping("/product/bad-products")
    public DataApiResult getListFiveBadProduct(){
        DataApiResult result=new DataApiResult();
        Pageable pageable=new PageRequest(0,5);
        Date date=new Date();
        List<ChartDataVM> chartDataVMS=productRepository.getListFiveBadProducts(pageable);
        result.setMessage("success");
        result.setSuccess(true);
        result.setData(chartDataVMS);
        return result;
